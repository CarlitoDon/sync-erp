...
- [x] T001 [Setup] Ensure feature directory structure exists for inventory, finance, and company
...
- [x] T002 [US1] Implement `inventoryService.ts` in `apps/web/src/features/inventory/services/inventoryService.ts`
...
- [x] T003 [US1] Create `GoodsReceiptModal.tsx` in `apps/web/src/features/inventory/components/GoodsReceiptModal.tsx`
...
- [x] T004 [US1] Create `StockAdjustmentModal.tsx` in `apps/web/src/features/inventory/components/StockAdjustmentModal.tsx`
  - Use `Dialog`. inputs: Product (select), Quantity, Type (Inc/Dec/Set), Reason.
- [x] T005 [US1] Integrate `GoodsReceiptModal` into `PurchaseOrders.tsx` in `apps/web/src/features/procurement/pages/PurchaseOrders.tsx`
  - Add "Receive" button to PO row actions or detail view.
- [x] T006 [US1] Integrate `StockAdjustmentModal` into `Inventory.tsx` in `apps/web/src/features/inventory/pages/Inventory.tsx`
...
- [x] T007 [US2] Update `paymentService.ts` in `apps/web/src/features/finance/services/paymentService.ts`
  - Add `getPaymentHistory(invoiceId)` calling `GET /api/payments/invoice/:id`.
- [x] T008 [US2] Create `PaymentHistoryList.tsx` in `apps/web/src/features/finance/components/PaymentHistoryList.tsx`
...
- [x] T009 [US2] Integrate `PaymentHistoryList` into `Invoices.tsx` (Details) in `apps/web/src/features/finance/pages/Invoices.tsx`
  - Add section below Invoice details. Pass `invoice.totalAmount` to component.
- [x] T010 [US2] Integrate `PaymentHistoryList` into `AccountsPayable.tsx` (Bill Details) in `apps/web/src/features/finance/pages/AccountsPayable.tsx`
...
- [x] T011 [US3] Create `userService.ts` in `apps/web/src/features/company/services/userService.ts`
...
- [x] T012 [US3] Create `InviteUserModal.tsx` in `apps/web/src/features/company/components/InviteUserModal.tsx`
  - Form: Email, Name. Calls `userService.invite`.
- [x] T013 [US3] Create `AssignCompanyModal.tsx` in `apps/web/src/features/company/components/AssignCompanyModal.tsx`
  - Form: Select Company (target), User (target). Calls `userService.assign`.
- [x] T014 [US3] Create `UserList.tsx` in `apps/web/src/features/company/components/UserList.tsx`
  - Table of users. Columns: Name, Email, Assigned Companies.
  - Action: "Assign to Company".
- [x] T015 [US3] Create `TeamManagement.tsx` page in `apps/web/src/features/company/pages/TeamManagement.tsx`
  - Layout: Header "Team Members" + "Invite User" button.
  - Body: `<UserList />`.
- [x] T016 [US3] Register path `/team` in `apps/web/src/app/AppRouter.tsx`
  - Route to `TeamManagement`.
- [x] T017 [US3] Add "Team" link to `apps/web/src/components/layout/Sidebar.tsx`
  - Icon: Users/Group icon.

## Phase 3: Polish

- [x] T018 [Polish] Verify `useConfirm` usage
  - Ensure critical actions (stock adjust, assign user) utilize the confirmation modal.
- [x] T019 [Polish] Verify Loading States
