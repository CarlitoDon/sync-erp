...
async function main(): Promise<void> {
  console.log('--- Seeding Kasur 100x200 (6 Units) for Santi Living ---');
  ...
  if (!product) {
    product = await prisma.product.create({
      data: {
        ...
        sku,
        name: 'Kasur 100x200',
        price: new Prisma.Decimal(600000), // Replacement / retail valuation
        averageCost: new Prisma.Decimal(450000), // Acquisition cost
  ...
  if (!rentalItem) {
    rentalItem = await prisma.rentalItem.create({
      data: {
        ...
        weeklyRate: new Prisma.Decimal(150000),
  ...
  if (unitsToCreateCount > 0) {
    ...
    while (newUnitsData.length < unitsToCreateCount) {
      const code = `KASUR100-${String(counter).padStart(3, '0')}`;
      if (!existingCodes.has(code)) {
        newUnitsData.push({
          ...
          sizeLabel: '100x200',
          acquisitionCost: new Prisma.Decimal(450000),
          acquiredAt: new Date(),
        });
      }
      counter++;
