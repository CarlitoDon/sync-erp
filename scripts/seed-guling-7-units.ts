...
async function main(): Promise<void> {
  console.log('--- Seeding Guling (7 Units) for Santi Living ---');
  ...
  if (!product) {
    product = await prisma.product.create({
      data: {
        ...
        sku,
        name: 'Guling Standar',
        price: new Prisma.Decimal(50000), // Replacement / retail valuation
  ...
  if (!rentalItem) {
    rentalItem = await prisma.rentalItem.create({
      data: {
        ...
        dailyRate: new Prisma.Decimal(7000),
        weeklyRate: new Prisma.Decimal(42000),
        monthlyRate: new Prisma.Decimal(90000),
  ...
  // 4. Check and create 7 physical RentalItemUnits
  ...
  if (unitsToCreateCount > 0) {
    ...
    while (newUnitsData.length < unitsToCreateCount) {
      const code = `GULING-${String(counter).padStart(3, '0')}`;
