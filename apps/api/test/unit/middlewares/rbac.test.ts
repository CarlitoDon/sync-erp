...
vi.mock('@sync-erp/database', async () => {
  const validation = await vi.importActual('../mocks/prisma.mock');
  return {
    prisma: (validation as any).mockPrisma,
  };
});
    ...
    it('should call next() if user has required permission', async () => {
      ...
      mockPrisma.companyMember.findUnique.mockResolvedValue({
        ...
        role: {
          ...
          name: 'Manager',
          permissions: [
            { permission: { module: 'orders', action: 'create' } },
          ],
        },
      });
  ...
  describe('RBACService', () => {
    ...
    describe('assignPermission', () => {
      it('should assign permission to role', async () => {
        ...
        (mockPrisma as any).rolePermission.create.mockResolvedValue(
          mockRolePermission
        );
    ...
    describe('seedDefaultPermissions', () => {
      it('should seed default permissions', async () => {
        ...
        expect(
          (mockPrisma as any).permission.upsert
