...
describe('RBAC authorization matrix', () => {
  ...
  it('denies every privileged endpoint to a non-admin with generic CRUD permissions', async () => {
    ...
    const endpoints: Array<[string, () => Promise<unknown>]> = [
      ...
      ['admin.getWebhookOutboxStats', () => adminRouter.createCaller(genericCrudCaller).getWebhookOutboxStats()],
      ['admin.listWebhookOutbox', () => adminRouter.createCaller(genericCrudCaller).listWebhookOutbox({ limit: 20, offset: 0 })],
      ['admin.getWebhookOutboxDetail', () => adminRouter.createCaller(genericCrudCaller).getWebhookOutboxDetail({ id: 'delivery' })],
      ['admin.replayWebhookOutbox', () => adminRouter.createCaller(genericCrudCaller).replayWebhookOutbox({ id: 'delivery' })],
      ['admin.replayWebhookOutboxBulk', () => adminRouter.createCaller(genericCrudCaller).replayWebhookOutboxBulk({ limit: 1 })],
