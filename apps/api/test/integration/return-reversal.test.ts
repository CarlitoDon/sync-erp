...
describe('US3: Sales Return Reversal', () => {
  ...
  it('should reverse COGS and increase stock when return is processed', async () => {
    ...
    const order = await salesOrderService.create(COMPANY_ID, 'user-1', {
      partnerId,
    ...
    const shippedProduct = await prisma.product.findUnique({ where: { id: productId } });
    expect(shippedProduct?.stockQty).toBe(8); // 10 - 2

    // 3. Process Return for 1 item
    // Reversal: 1 * 150,000 = 150,000
    await salesOrderService.returnOrder(COMPANY_ID, order.id, [{ productId, quantity: 1 }]);

    // 4. Verify Stock Increased
    const returnedProduct = await prisma.product.findUnique({ where: { id: productId } });
    ...
    // Find Return Journal
    const returnJournal = journals.find((j) => j.reference?.includes('Return for order')) as any;

    expect(returnJournal).toBeDefined();

    // Verify Lines: Dr 1400 (Asset), Cr 5000 (COGS)
    const assetLine = returnJournal.lines.find((l: any) => l.account.code === '1400');
    const cogsLine = returnJournal.lines.find((l: any) => l.account.code === '5000');
