...
async function main() {
  ...
  for (const company of companies) {
    console.log(`Checking accounts for company: ${company.name} (${company.id})...`);

    // 2. Run seedDefaultAccounts for each
    // The service method is idempotent (checks existence before creating)
    const accounts = await accountService.seedDefaultAccounts(company.id);

    console.log(`  - Ensured ${accounts.length} default accounts exist.`);

    // 3. Specifically verify 2105
    const acc2105 = await accountService.getByCode(company.id, '2105');
    if (acc2105) {
