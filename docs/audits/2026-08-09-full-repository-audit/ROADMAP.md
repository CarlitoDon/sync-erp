...
Current update date: 2026-08-13. This roadmap is dependency-ordered. Time ranges are planning horizons, not promises; owners must size work after containment and environment access are established. The 2026-08-09 audit baseline and each dated section below are time-bounded snapshots; this update does not rewrite historical evidence.
...
## 2026-08-13 — CICD-003 SSH-pinning code-remediation evidence

[PR #84](https://github.com/CarlitoDon/sync-erp/pull/84) is open at the observed head `e1f377361271450a789884554ef7ffbd665e6c71`. The branch adds a fail-closed Hostinger SSH pinning helper and wires the API deployment, MCP deployment, and staging rollback-drill paths to require an exact Hostinger host/port `known_hosts` input. `HOSTINGER_SSH_KNOWN_HOSTS` is a design/required-input contract only: the helper fails closed when the input is missing or invalid, but this branch does not prove that the secret exists, contains a provider-approved identity, or was used live.

The [CI/CD run #31693634711](https://github.com/CarlitoDon/sync-erp/actions/runs/31693634711) passed its repository quality jobs; the [Quality Gates (API) job](https://github.com/CarlitoDon/sync-erp/actions/runs/31693634711/job/94426318093) reports `Test Hostinger SSH pinning` passed. The [Playwright E2E run #31693634657](https://github.com/CarlitoDon/sync-erp/actions/runs/31693634657) also passed. The PR run's `Deploy API to Hostinger` and `Deploy Web to Vercel` jobs were skipped because it was a pull-request check, so no live Hostinger connection or deployment is claimed.

This is code/CI evidence only. The `HOSTINGER_SSH_KNOWN_HOSTS` reference remains a design/required-input contract; its value was not read. Provider/OOB verification of the actual Hostinger identity, secret configuration/update, live pinning, deployment, rollback, production closure, and credential-incident closure remain unproven. Follow the [Hostinger SSH host-key rotation runbook](../../runbooks/hostinger-ssh-key-rotation.md) before treating the control as operationally active.

The current workflow contract is also bounded to source evidence: `.github/workflows/ci-cd.yml` triggers on pull request, push, and manual dispatch; `changes`, `ci-api`, and `ci-web` run for those events; `deploy_api` runs only on `push`, needs `changes` plus `ci-api`, and does not consume the `api` filter output; `deploy_web` runs only on `push`, needs `changes` plus `ci-web`, and requires the `web` filter to be `true`. Manual dispatch on `ci-cd.yml` has no deployment-environment input and does not run API/Web deploy jobs. MCP uses path-filtered pushes plus a manual `staging`/`production` choice; its source logic resolves `main` to production and does not restrict a manually selected production environment to a particular ref. The rollback drill is manual-only, staging-only, and accepts `dev` or `codex/*` refs. The API build is produced at `apps/api/dist`, while the runner-built production dependency tree is assembled under `deploy/api-mcp/node_modules/`; none of these source contracts proves a live deployment. The `HOSTINGER_SSH_KNOWN_HOSTS` reference remains design/required-input only until provider/OOB verification and live deployment evidence exist.
...
- [ ] Restrict production deployment/manual dispatch while Hostinger and branch gates are unsafe; complete provider/OOB host-identity verification before the next credential-bearing transfer. PR #84 provides code/CI evidence for the fail-closed pinning path, but provider/OOB verification, the configured secret, and a live connection remain unproven.
