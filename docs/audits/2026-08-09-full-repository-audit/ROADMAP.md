...
Status date: 2026-08-11. This roadmap is dependency-ordered. Time ranges are planning horizons, not promises; owners must size work after containment and environment access are established.
...
Current `origin/dev` baseline as of 2026-08-11 is merge commit [`dc82886c77730f4a32442feeb50476d8c518a3b1`](https://github.com/CarlitoDon/sync-erp/commit/dc82886c77730f4a32442feeb50476d8c518a3b1), merged from [PR #67](https://github.com/CarlitoDon/sync-erp/pull/67) at `2026-08-11T04:46:56Z` with PR head `54bbc3b344d415ea11baf5a0bb54af929e58783c`.
...
- **Open operational follow-up:** Ops found pre-existing release-state/current-symlink drift: `current` points at `27f8fb85` while PM2/release identity serves `5b367b1e`. This is not attributed to PR #67 and must be reconciled before the next deployment.
- **Post-merge result:** code/quality is green, but staging API deployment is red. No production stability, production closure, or launch readiness is claimed.
...
- [x] Reconcile Prisma migration authority and reproduce `migrate deploy` against a fresh empty database and a production-like snapshot; the staging upgrade path reported 16 migrations and no pending migrations in run `31359645256`. The disposable fresh path failure on `20250505160000_generalize_webhook_outbox` dependency order has been resolved via idempotent SQL blocks (`IF EXISTS`), and staging drift has been captured in `20260810085025_sync_drift_webhook_outbox` to align `schema.prisma` safely without renaming staging folders.

### Blocked pending external authority

- [ ] Replace in-place Hostinger deployment with immutable SHA releases, checksum validation, isolated-port readiness, atomic switch, retained previous release, and explicit PM2 rollback; the staging canary exercised the SHA release/retention path and kept previous release `31e9cc5163281608861a6314fe8d8fab105167b4`, and the staging rollback drill ([run #31378213027](https://github.com/CarlitoDon/sync-erp/actions/runs/31378213027)) passed the failure-injected release/rollback plus external restored-release identity/health steps. Production rollback evidence remains open.
...
- [ ] Confirm staging database connectivity, reconcile the pre-existing release-state/current-symlink drift (`current` points at `27f8fb85` while PM2/release identity serves `5b367b1e`), and rerun staging deployment through the approved workflow. This remains open because no deployment retry was performed after PR #67.
...
- [ ] **Phase 1 exit criteria:** Fresh and upgrade migrations pass; staging deploy and rollback drill pass; all required checks fail when deliberately broken; real Playwright specs execute; and the exact release SHA plus health/readiness are observable. Fresh/upgrade migration and the staging rollback drill are now closed; required-check enforcement, Playwright execution, and readiness observability remain open.
