...
The release-readiness audit snapshot before this documentation-only evidence correction used `origin/dev` at merge commit [`65a040f97157c45870f25ca6d33c51f86a39a153`](https://github.com/CarlitoDon/sync-erp/commit/65a040f97157c45870f25ca6d33c51f86a39a153), merged from [PR #76](https://github.com/CarlitoDon/sync-erp/pull/76) at `2026-08-12T10:50:52Z`. The immediately preceding readiness/deployment fixes are merged through [PR #70](https://github.com/CarlitoDon/sync-erp/pull/70), [PR #71](https://github.com/CarlitoDon/sync-erp/pull/71), [PR #72](https://github.com/CarlitoDon/sync-erp/pull/72), [PR #73](https://github.com/CarlitoDon/sync-erp/pull/73), [PR #74](https://github.com/CarlitoDon/sync-erp/pull/74), and [PR #75](https://github.com/CarlitoDon/sync-erp/pull/75). This is an evidence snapshot, not a promise that a later documentation-only merge has the same commit SHA.

## 2026-08-12 — PR #70–#75 readiness, deployment verifier, and staging evidence
...
- **Release-identity boundary:** The latest verified API staging release is `65a040f…`; the standalone MCP handshake evidence is from `d512ac6…`. These are intentionally recorded separately because the roadmap PR changed documentation and did not redeploy the standalone MCP release. The attempted public host `mcp-staging.santiliving.com` did not resolve during the spot-check, so public MCP DNS/edge readiness remains open.
...
- [x] Verify the staging readiness contract and the non-mutating standalone MCP protocol handshake. Evidence: PRs #70–#73 added and corrected the gates; [CI/CD run #31589227448](https://github.com/CarlitoDon/sync-erp/actions/runs/31589227448) deployed and verified API SHA `65a040f97157c45870f25ca6d33c51f86a39a153`; [standalone MCP run #31574608473](https://github.com/CarlitoDon/sync-erp/actions/runs/31574608473) passed unauthenticated 401, authenticated `initialize`, `tools/list`, and session `DELETE` without a tool call. Production and public MCP edge evidence remain open.
- [x] Reconcile Prisma migration authority and reproduce `migrate deploy` against a fresh empty database and a production-like snapshot; the earlier staging upgrade path reported 16 migrations and no pending migrations in run `31359645256`, while the current `65a040f…` deployment found 18 migrations and no pending migrations. The disposable fresh path failure on `20250505160000_generalize_webhook_outbox` dependency order has been resolved via idempotent SQL blocks (`IF EXISTS`), and staging drift has been captured in `20260810085025_sync_drift_webhook_outbox` to align `schema.prisma` safely without renaming staging folders.

### Blocked pending external authority

- [ ] Replace in-place Hostinger deployment with immutable SHA releases, checksum validation, isolated-port readiness, atomic switch, retained previous release, and explicit PM2 rollback; staging now has exact-SHA release/retention and active-state verification in [PR #75](https://github.com/CarlitoDon/sync-erp/pull/75) and [run #31587164909](https://github.com/CarlitoDon/sync-erp/actions/runs/31587164909), plus the post-merge verification in [run #31589227448](https://github.com/CarlitoDon/sync-erp/actions/runs/31589227448), while the earlier failure-injected staging rollback drill ([run #31378213027](https://github.com/CarlitoDon/sync-erp/actions/runs/31378213027)) also passed. Production rollback evidence remains open.
