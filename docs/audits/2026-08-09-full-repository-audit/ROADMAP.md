...
Current `origin/dev` baseline as of 2026-08-12 is merge commit [`533845fa6b4090b1c3898d5fea4d63fe1212358a`](https://github.com/CarlitoDon/sync-erp/commit/533845fa6b4090b1c3898d5fea4d63fe1212358a), merged from [PR #75](https://github.com/CarlitoDon/sync-erp/pull/75) at `2026-08-12T10:22:57Z`. The immediately preceding readiness/deployment fixes are merged through [PR #70](https://github.com/CarlitoDon/sync-erp/pull/70), [PR #71](https://github.com/CarlitoDon/sync-erp/pull/71), [PR #72](https://github.com/CarlitoDon/sync-erp/pull/72), [PR #73](https://github.com/CarlitoDon/sync-erp/pull/73), and [PR #74](https://github.com/CarlitoDon/sync-erp/pull/74).
...
- **Release-identity boundary:** API staging is currently on `533845fa…`; the standalone MCP handshake evidence is from `d512ac6…`. These are intentionally recorded separately because PR #75 changed API deployment verification and did not redeploy the standalone MCP release. The attempted public host `mcp-staging.santiliving.com` did not resolve during the current spot-check, so public MCP DNS/edge readiness remains open.
...
- [x] Verify the staging readiness contract and the non-mutating standalone MCP protocol handshake. Evidence: PRs #70–#73 added and corrected the gates; [CI/CD run #31587164909](https://github.com/CarlitoDon/sync-erp/actions/runs/31587164909) deployed and verified API SHA `533845fa6b4090b1c3898d5fea4d63fe1212358a`; [standalone MCP run #31574608473](https://github.com/CarlitoDon/sync-erp/actions/runs/31574608473) passed unauthenticated 401, authenticated `initialize`, `tools/list`, and session `DELETE` without a tool call. Production and public MCP edge evidence remain open.
- [x] Reconcile Prisma migration authority and reproduce `migrate deploy` against a fresh empty database and a production-like snapshot; the earlier staging upgrade path reported 16 migrations and no pending migrations in run `31359645256`, while the current `533845fa…` deployment found 18 migrations and no pending migrations. The disposable fresh path failure on `20250505160000_generalize_webhook_outbox` dependency order has been resolved via idempotent SQL blocks (`IF EXISTS`), and staging drift has been captured in `20260810085025_sync_drift_webhook_outbox` to align `schema.prisma` safely without renaming staging folders.

### Blocked pending external authority

- [ ] Replace in-place Hostinger deployment with immutable SHA releases, checksum validation, isolated-port readiness, atomic switch, retained previous release, and explicit PM2 rollback; staging now has exact-SHA release/retention and active-state verification in [PR #75](https://github.com/CarlitoDon/sync-erp/pull/75) and [run #31587164909](https://github.com/CarlitoDon/sync-erp/actions/runs/31587164909), while the earlier failure-injected staging rollback drill ([run #31378213027](https://github.com/CarlitoDon/sync-erp/actions/runs/31378213027)) also passed. Production rollback evidence remains open.
