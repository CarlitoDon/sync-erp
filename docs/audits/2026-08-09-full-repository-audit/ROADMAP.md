...
Current update date: 2026-08-14. This roadmap is dependency-ordered. Time ranges are planning horizons, not promises; owners must size work after containment and environment access are established. The 2026-08-09 audit baseline and each dated section below are time-bounded snapshots; this update does not rewrite historical evidence.
...
The release-readiness audit is recorded as time-bounded evidence snapshots. [PR #76](https://github.com/CarlitoDon/sync-erp/pull/76) merged at `65a040f97157c45870f25ca6d33c51f86a39a153`, [PR #77](https://github.com/CarlitoDon/sync-erp/pull/77) merged at `a2265bccd6a8a9f7ea7cc5b01205cd436e55e054`, [PR #78](https://github.com/CarlitoDon/sync-erp/pull/78) merged at `2fea99d950f416742413557c15cd5ac094bde882`, and [PR #79](https://github.com/CarlitoDon/sync-erp/pull/79) merged at `ac0c45a4ee19d657cffde746c1a04b46dcab04aa`; their staging deployments are recorded below. The immediately preceding readiness/deployment fixes are merged through [PR #70](https://github.com/CarlitoDon/sync-erp/pull/70), [PR #71](https://github.com/CarlitoDon/sync-erp/pull/71), [PR #72](https://github.com/CarlitoDon/sync-erp/pull/72), [PR #73](https://github.com/CarlitoDon/sync-erp/pull/73), [PR #74](https://github.com/CarlitoDon/sync-erp/pull/74), and [PR #75](https://github.com/CarlitoDon/sync-erp/pull/75). These are evidence snapshots; the latest PR #79 retry snapshot below remains staging evidence and does not claim Phase 1 or production closure.

Documentation-only [PR #80](https://github.com/CarlitoDon/sync-erp/pull/80) merged to `dev` as `b8b4aa8394b9afc37aaadc2c24d65b1ddc19fbc8`; post-merge [CI/CD run #31601173883](https://github.com/CarlitoDon/sync-erp/actions/runs/31601173883) and [Playwright E2E run #31601173885](https://github.com/CarlitoDon/sync-erp/actions/runs/31601173885) completed successfully. This records merge/pipeline evidence only and does not claim Phase 1 or production closure.

## 2026-08-13 — CICD-002 code-remediation evidence

[PR #82](https://github.com/CarlitoDon/sync-erp/pull/82) merged as `3b6b4cff31abe98d788bc579d1e72e22e6e053df` from head `ccd8cbff55a88f4243515753df25c6bd99a65a9d`. Post-merge [CI/CD run #31656610112](https://github.com/CarlitoDon/sync-erp/actions/runs/31656610112) and [E2E run #31656610048](https://github.com/CarlitoDon/sync-erp/actions/runs/31656610048) passed, and the focused security contract `node --test scripts/ai-review-security.test.mjs` passed 17/17. The merged code proves a trusted `pull_request_target`/base-SHA-only analyzer/publisher split: the analyzer has the AI secret and `pull-requests: read`; the publisher has `pull-requests: write` and no AI key; the handoff is a bounded, schema-validated artifact; publication is advisory `COMMENT` with `commit_id`; actions are pinned and job timeouts are set; and static/adversarial tests are wired into CI. This is repository/CI evidence only: live `pull_request_target`/provider execution, secretless runtime logs, branch protection/CODEOWNERS/required-check enforcement, external provider ownership/TLS/logging/availability, atomic head-race prevention, Phase 1 sign-off, and production closure remain unproven.

## 2026-08-14 — CICD-003 SSH-pinning operational evidence

[PR #84](https://github.com/CarlitoDon/sync-erp/pull/84) merged to `dev` as `c862f5a2a695a03ffa0428f2f8f8a66a937b5b8b` from head `18a465fcf42acb1be2698dd0f11e4eca80f667cc`. It replaced permissive host-key acceptance with a fail-closed helper shared by API deployment, standalone MCP deployment, and the staging rollback drill. Authenticated cPanel/server information and an independent out-of-band comparison confirmed the exact shared Hostinger target, port `65002`, and required raw `ssh-ed25519` identity without recording key or fingerprint material. The reviewed value was installed as the GitHub Actions secret `HOSTINGER_SSH_KNOWN_HOSTS`; only its update metadata, at `2026-08-14T00:32:07Z`, is recorded here.

Post-merge [CI/CD run #31758446770](https://github.com/CarlitoDon/sync-erp/actions/runs/31758446770) deployed API release `c862f5a2…` with strict pinning, 18 recognized migrations and no pending migration, and successful local/public API and MCP health plus OAuth probes. The shared contract was also consumed successfully by [standalone MCP run #31758446761](https://github.com/CarlitoDon/sync-erp/actions/runs/31758446761). A rollback drill then exposed pre-existing `current`-symlink drift. [PR #85](https://github.com/CarlitoDon/sync-erp/pull/85), merged as `9e77f4c1e612ee8092bdcbc935cb5a1bfcd4699d`, repaired atomic replacement and added the regression test. Its post-merge [CI/CD run #31760295823](https://github.com/CarlitoDon/sync-erp/actions/runs/31760295823) deployed exact release `9e77f4c1…` successfully.

The final staging [rollback drill #31761990061](https://github.com/CarlitoDon/sync-erp/actions/runs/31761990061) completed successfully on exact release `9e77f4c1…`, reported no active-release drift warning, skipped migrations, and restored the same release with external API and MCP health checks passing. CICD-003 is therefore operationally closed for the shared Hostinger endpoint and these three staging workflow paths. This does not prove a production application deployment or production rollback, does not close the broader credential incident, and does not satisfy the remaining Phase 1 or production release gates.

The current workflow contract remains bounded in other respects: `.github/workflows/ci-cd.yml` triggers on pull request, push, and manual dispatch; `deploy_api` runs on every qualifying push without consuming the `api` filter; manual dispatch on that workflow does not deploy; MCP manual routing can still select production independently of the ref; and the rollback drill is staging-only. The evidence above proves live use of the strict Hostinger trust input and staging behavior, not production deployment governance.

## 2026-08-12 — PR #70–#79 readiness, deployment verifier, and staging evidence
...
- **Post-PR #79 staging deployment retry for merge `ac0c45a4ee19d657cffde746c1a04b46dcab04aa`:** Original [CI/CD run #31598210290](https://github.com/CarlitoDon/sync-erp/actions/runs/31598210290) passed API/Web quality gates, Playwright E2E, and Vercel; its first [Hostinger job #94121576972](https://github.com/CarlitoDon/sync-erp/actions/runs/31598210290/job/94121576972) failed on the initial SSH/SCP connection to port `65002` with a timeout before any remote release transaction. This was a transient provider/runner connectivity failure, not a code regression. The authorized deploy-only rerun [job #94123872796](https://github.com/CarlitoDon/sync-erp/actions/runs/31598210290/job/94123872796) succeeded: it validated the exact SHA `ac0c45a4ee19d657cffde746c1a04b46dcab04aa`, found 18 migrations with `No pending migrations to apply.`, aligned the pre-existing active-release/current-symlink drift, passed local/public health and OAuth loopback/public-edge checks, activated the SHA-named release, and retained previous release `27f8fb85d66be21928310f2d9b85d3c233697524`. Final probes at `2026-08-12T13:09:33Z` returned HTTP 200 for `/health`, `/mcp/health`, and `/readyz` with exact release `ac0c45a4ee19d657cffde746c1a04b46dcab04aa`. This is a time-bounded staging snapshot only; production deployment/rollback, public MCP DNS/edge availability, required-check/review enforcement, and broader browser/business smoke remain open. The non-mutating standalone MCP `initialize`/`tools/list` handshake remains separate in [run #31574608473](https://github.com/CarlitoDon/sync-erp/actions/runs/31574608473) against release `d512ac6e7c83eaa1e0f10839c669a624d3378ca3`.
...
- **Release-identity boundary:** The latest recorded API staging release is `ac0c45a…` (the preceding PR #78 snapshot was `2fea99d…`); the standalone MCP handshake evidence is from `d512ac6…`. These are intentionally recorded separately because the roadmap PRs changed documentation and did not redeploy the standalone MCP release. The attempted public host `mcp-staging.santiliving.com` did not resolve during the spot-check, so public MCP DNS/edge readiness remains open.
- **Still open at this 2026-08-12 snapshot:** production deployment and rollback proof, production database/migration authority, required-check/review enforcement, browser/business smoke beyond the available E2E signal, public MCP edge availability, provider-side SSH host-key controls, credential incident closure, and the other Phase 0–5 gates below. The permissive SSH host-key setup described by that historical snapshot is retained as baseline evidence; the later 2026-08-13 PR #84 code/CI follow-up above does not prove provider/OOB verification, secret configuration, live SSH, deployment, rollback, or production readiness.
...
- [x] Verify the shared Hostinger endpoint out of band, install the reviewed host-identity pin through the secret manager, and prove fail-closed strict pinning before transfer in API, MCP, and staging rollback workflows. Evidence: PRs #84/#85 and runs #31758446770, #31758446761, #31760295823, and #31761990061. No key, fingerprint, or secret value is recorded.
- [ ] Restrict production deployment/manual dispatch and enforce branch/environment review gates. CICD-003 host-identity pinning is closed for the shared endpoint, but production routing and governance remain separate open controls.
...
- [x] Expose release identity through API `/health` and MCP `/health` as `release.commit` and `release.version`, with explicit `unknown` fallbacks when `release.json` is absent; deployment health checks compare the expected `GITHUB_SHA` on local and external API responses and local MCP responses. The 2026-08-12 staging deployments and spot-checks prove the API release identity and readiness contract through the latest `ac0c45a…` retry snapshot; production evidence remains open.
- [x] Add a tested API Hostinger rollback transaction: capture the previous release/PM2 metadata, stage a SHA-named release, keep the previous app active through forward-only migration, restore the previous PM2 release after local startup/health/identity failure, and fail closed without a previous release. Staging rollback is proven by runs #31378213027 and #31761990061; production rollback remains open.
...
- [x] Verify the staging readiness contract and the non-mutating standalone MCP protocol handshake. Evidence: PRs #70–#73 added and corrected the gates; the deploy-only retry [job #94123872796](https://github.com/CarlitoDon/sync-erp/actions/runs/31598210290/job/94123872796) deployed and verified API SHA `ac0c45a4ee19d657cffde746c1a04b46dcab04aa`; [standalone MCP run #31574608473](https://github.com/CarlitoDon/sync-erp/actions/runs/31574608473) passed unauthenticated 401, authenticated `initialize`, `tools/list`, and session `DELETE` without a tool call. Production and public MCP edge evidence remain open.
- [x] Reconcile Prisma migration authority and reproduce `migrate deploy` against a fresh empty database and a production-like snapshot; the earlier staging upgrade path reported 16 migrations and no pending migrations in run `31359645256`, while the PR #78 deployment (`2fea99d…`) and PR #79 retry (`ac0c45a…`) each found 18 migrations and no pending migrations. The disposable fresh path failure on `20250505160000_generalize_webhook_outbox` dependency order has been resolved via idempotent SQL blocks (`IF EXISTS`), and staging drift has been captured in `20260810085025_sync_drift_webhook_outbox` to align `schema.prisma` safely without renaming staging folders.
- [ ] **CICD-002 residual acceptance:** Code remediation is merged and verified in [PR #82](https://github.com/CarlitoDon/sync-erp/pull/82) as `3b6b4cff31abe98d788bc579d1e72e22e6e053df` (head `ccd8cbff55a88f4243515753df25c6bd99a65a9d`), with [CI/CD run #31656610112](https://github.com/CarlitoDon/sync-erp/actions/runs/31656610112), [E2E run #31656610048](https://github.com/CarlitoDon/sync-erp/actions/runs/31656610048), and the focused security contract at 17/17. Full acceptance remains open while live PR/provider/runtime, control-plane, head-race, Phase 1, and production evidence gates are unproven.

### Blocked pending external authority

- [ ] Replace in-place Hostinger deployment with immutable SHA releases, checksum validation, isolated-port readiness, atomic switch, retained previous release, and explicit PM2 rollback. Staging has exact-SHA release/retention evidence from PR #75 and runs #31587164909, #31589227448, #31593365072, and #31595432040. PR #85 repaired existing-symlink replacement; [run #31760295823](https://github.com/CarlitoDon/sync-erp/actions/runs/31760295823) deployed release 9e77f4c1…, and [rollback drill #31761990061](https://github.com/CarlitoDon/sync-erp/actions/runs/31761990061) showed no release-state drift and restored the same release successfully. Production deployment and rollback evidence remain open, so this broader item stays unchecked.
