...
- [ ] Configure Google OAuth with environment secrets `SYNC_ERP_AUTH_STATE_SECRET`, `GOOGLE_OAUTH_CLIENT_ID`, and `GOOGLE_OAUTH_CLIENT_SECRET`; authorize both callback URIs exactly: `https://api-sync-erp.khusnudhoni.online/api/auth/google/callback` and `https://api-staging-sync-erp.khusnudhoni.online/api/auth/google/callback`.
